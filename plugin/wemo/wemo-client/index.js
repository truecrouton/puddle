...
class Wemo {
  ...
  /**
 * Converts a dotted IP string (192.168.1.1) to a 32-bit integer
 */
  ipToInt = (ip) =>
  ...
  getLocalInterfaceAddress(targetNetwork) {
    ...
    const wemoInt = this.ipToInt(targetNetwork);

    for (const name of Object.keys(interfaces)) {
      for (const iface of interfaces[name]) {
        if (iface.family === 'IPv4' && !iface.internal) {
          const hostInt = this.ipToInt(iface.address);
          const maskInt = this.ipToInt(iface.netmask);

          // The Magic Logic: (IP AND Mask) == (WemoIP AND Mask)
          if ((hostInt & maskInt) === (wemoInt & maskInt)) {
