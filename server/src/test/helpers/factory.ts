...
import { randAbbreviation, randAccessory, randNumber, randUserName, randVerb, randWord } from '@ngneat/falso';
...
export function topicGenerate(baseTopic: string = 'zigbee2mqtt'): string {
    return `${baseTopic.length ? baseTopic + '/' : ''}${randWord()}_${randAbbreviation()}`.toLocaleLowerCase();
}